...
- reads the eight Octoliner values into a list and identifies which sensor (1 through 8) sits on top of the line
- uses that 1-based index to flash both LEDs on and off the same number of times, pausing for two seconds between each flash
- stops flashing once the chosen count finishes so the total blinks match the sensor position
- passes when the `line_sensor_leds` verification_function detects exactly six flashes with roughly two seconds between each on-event
